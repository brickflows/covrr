...
export const SelectionBox = memo(
  ({ onResizeHandlePointerDown, onConnectionStart }: SelectionBoxProps) => {
    const soleLayerId = useSelf((me) =>
      me.presence.selection.length === 1 ? me.presence.selection[0] : null,
    );

    // Get the selected layer to check if it's a sent message
    ...
    return (
      <>
        ...
        {isShowingHandles &&
         ...
         onConnectionStart && (
          <>
            {/* Connection handle on the left (output) - for all messages */}
            <ConnectionHandle
              layerId={soleLayerId}
              x={bounds.x - 20}
              y={bounds.y + bounds.height / 2}
              onConnectionStart={(id, _) => {
                // When starting from the left handle, we actually start from the right side
